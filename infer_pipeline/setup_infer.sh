...
echo "[Step 2.6] Installing nvidia-docker2 runtime..."
sudo apt-get update
...
cd ~/leximind 
sudo docker build -t llama-env:py310 .
...
sudo docker rm -f llama-train || true

sudo docker run --gpus all -it --name llama-train \
  -e WANDB_API_KEY=$WANDB_API_KEY \
  -v ~/llama-factory:/llama-factory \
  llama-env:py310 \
  bash

sudo docker exec -i llama-train bash -c "bash /llama-factory/fix_dependencies.sh"
